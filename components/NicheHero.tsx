...
const NicheHero = ({ niche }: HeroProps) => {
  ...
  const badgeText = config
    ? `Performance-Based Acquisition for ${displayName}`
    : 'Performance-Based Acquisition';

  return (
    <section className="relative pt-16 pb-24 px-6 max-w-6xl mx-auto flex flex-col items-center">
      {/* Overhead Badge */}
      ...
      <h1 className="text-4xl sm:text-5xl md:text-[72px] lg:text-[84px] font-bold tracking-[-0.03em] leading-[1.05] text-center text-text max-w-5xl mb-6">
        High-Converting Funnels for {displayName}<br />
        <span className="text-neutral-300">$0 Upfront — <span className="font-serif italic font-extralight text-accent">Pay When You Win</span></span>
      </h1>
      ...
      <div className="flex flex-wrap justify-center gap-3 md:gap-4 text-neutral-400 font-mono text-xs md:text-sm mb-20">
        <span className="px-4 py-1.5 bg-black/40 rounded-md border border-neutral-800 shadow-sm">&lt;For {displayName} /&gt;</span>
